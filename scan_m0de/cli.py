...
def main():
    parser = argparse.ArgumentParser(
        description="scan-m0de: Local network scanner, cataloger, and TUI device manager."
    )
    parser.add_argument(
        "--db",
        type=Path,
        default=None,
        help="Path to SQLite database file (default: ~/.local/share/scan-m0de/devices.db)",
    )
    parser.add_argument("--subnet", type=str, help="Target IPv4 subnet CIDR (e.g. 192.168.1.0/24)")
    parser.add_argument(
        "--scan-only",
        action="store_true",
        help="Run scan headlessly in background without launching TUI",
    )
    parser.add_argument(
        "--export-json", type=Path, help="Export device catalog to JSON file and exit"
    )
    parser.add_argument(
        "--export-csv", type=Path, help="Export device catalog to CSV file and exit"
    )
    parser.add_argument(
        "--update-oui",
        action="store_true",
        help="Download/update the IEEE OUI vendor database and exit",
    )
    ...
    if args.update_oui:
        from scan_m0de.oui import get_oui_stats, update_ieee_db

        print("📡 Downloading IEEE OUI vendor database...")
        if update_ieee_db():
            stats = get_oui_stats()
            print(
                f"✅ OUI database updated! {stats['ieee_count']:,} IEEE entries + {stats['friendly_count']} curated friendly names"
            )
        else:
            print(
                "❌ Failed to download OUI database. Check your internet connection.",
                file=sys.stderr,
            )
            sys.exit(1)
        sys.exit(0)
    ...
    if args.oui_stats:
        from scan_m0de.oui import CACHE_FILE, get_oui_stats

        stats = get_oui_stats()
        print("📊 OUI Database Stats:")
        ...
        if stats["cache_age_days"] >= 0:
            print(
                f"   Cache age:               {stats['cache_age_days']} days {'(stale — run --update-oui)' if stats['is_stale'] else '(fresh)'}"
            )
            print(f"   Cache location:          {CACHE_FILE}")
        else:
            print("   Cache:                   Not downloaded yet (run --update-oui)")
        sys.exit(0)
    ...
    if args.scan_only:
        print("⚡ Running headless network scan...")
        scanner = NetworkScanner(args.subnet)
        devices = scanner.scan()
        summary = db.sync_scan_results(devices, scanner.subnet)
        print(
            f"✅ Headless scan complete! Found {summary['total_found']} devices ({summary['new_count']} NEW, {summary['diff_count']} diff events)."
        )
        sys.exit(0)
